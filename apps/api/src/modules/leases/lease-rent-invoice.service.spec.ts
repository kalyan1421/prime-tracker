...
describe('summariseInvoices (pure)', () => {
  ...
  it('zeroes cleanly on an empty ledger', () => {
    expect(summariseInvoices([], d('2026-05-15'))).toEqual({
      ...
      outstanding: 0,
      overdueCount: 0,
      // R22 — how many months were billed from a period corrected afterwards.
      flaggedCount: 0,
    });
  });
});
...
/**
 * A month voided by endTenancy: unpaid, and carrying the voidedAt the DB CHECK
 * `invoice_void_requires_voided_at` insists on. The stamps matter to the tests — a
 * silent un-void leaves them on the row, which is what makes the resulting DUE month
 * unfindable rather than merely wrong.
 */
const voidedRow = (over: Record<string, unknown> = {}) =>
  invoiceRow({
    status: 'VOID',
    voidedAt: d('2026-03-31'),
    voidReason: 'Tenancy ended 2026-03-15',
    ...over,
  });
...
describe('LeaseRentInvoiceService.generateDueThrough (cron)', () => {
  ...
  it('walks every ACTIVE, non-deleted lease and appends the missing months', async () => {
    ...
    // `terminationDate: null` keeps an ended tenancy out of the nightly run even if
    // someone left its status on ACTIVE — a wrong invoice generated every night is
    // worse than one generated by hand.
...
describe('LeaseRentInvoiceService.recordPayment — the mirror', () => {
  ...
  it('refuses to collect against a VOIDED month', async () => {
    // Without the guard the row is re-derived to PARTIAL/PAID while voidedAt still says
    // it was voided — a collection against a month the tenant was not in occupation for.
    mockPrisma.leaseRentInvoice.findUnique.mockResolvedValue(voidedRow());

    await expect(service.recordPayment('inv-1', { amountPaid: 5000 })).rejects.toThrow(
      BadRequestException,
    );
    await expect(service.recordPayment('inv-1', { amountPaid: 5000 })).rejects.toThrow(
      /voided when the tenancy ended/,
    );
    expect(mockPrisma.leaseRentInvoice.update).not.toHaveBeenCalled();
  });
...
describe('LeaseRentInvoiceService.clearPayment', () => {
  ...
  it('is NOT the equivalent path for a void — clearing a voided month is refused', async () => {
    // The reported failure exactly: a tenant moves out in March, endTenancy voids
    // April..December, Finance opens April and clicks Clear. deriveInvoiceStatus knows
    // nothing about VOID, so the row came back as DUE with voidedAt still on it —
    // outstanding AR on an ended lease, and it reads as overdue.
    mockPrisma.leaseRentInvoice.findUnique.mockResolvedValue(voidedRow());

    await expect(service.clearPayment('inv-1')).rejects.toThrow(BadRequestException);
    await expect(service.clearPayment('inv-1')).rejects.toThrow(/never owed/);
    expect(mockPrisma.leaseRentInvoice.update).not.toHaveBeenCalled();
  });

  it('names the move-out date as the fix, and does not offer an un-void', async () => {
    mockPrisma.leaseRentInvoice.findUnique.mockResolvedValue(voidedRow());
    await expect(service.clearPayment('inv-1')).rejects.toThrow(/move-out date/);
  });
});
...
describe('LeaseRentInvoiceService.waive', () => {
  ...
  it('refuses to waive a VOIDED month — there is nothing owed to forgive', async () => {
    // Waiving is milder than the other two but no safer: it overwrites VOID with WAIVED
    // and the month stops reading as "never owed" at all.
    mockPrisma.leaseRentInvoice.findUnique.mockResolvedValue(voidedRow());

    await expect(service.waive('inv-1', 'goodwill')).rejects.toThrow(BadRequestException);
    await expect(service.waive('inv-1', 'goodwill')).rejects.toThrow(/never owed/);
    expect(mockPrisma.leaseRentInvoice.update).not.toHaveBeenCalled();
  });
});

/**
 * Voiding is one-way, and the three collection paths are the only ways back.
 *
 * Grouped deliberately: the bug was that each of them re-derived a status through
 * `deriveInvoiceStatus`, which has no VOID branch, so each silently answered DUE and
 * put a month the tenant never owed back into AR — with voidedAt still populated, so
 * nothing downstream flagged it. The WAIVED cases sit alongside as the regression
 * guard: that status stays reversible, and clearPayment is still its only lift.
 */
describe('LeaseRentInvoiceService — a VOID month cannot be resurrected', () => {
  let service: LeaseRentInvoiceService;
  ...
  it('refuses on every write path, and issues no update at all', async () => {
    await expect(service.recordPayment('inv-1', { amountPaid: 5000 })).rejects.toThrow(
      BadRequestException,
    );
    await expect(service.clearPayment('inv-1')).rejects.toThrow(BadRequestException);
  ...
  it('refuses a FREE month that was voided, not just a DUE one', async () => {
    // voidAfter voids FREE months too — an abatement scheduled for a term that did not
    // run. recordPayment would otherwise reach the free-rent guard and report the wrong
    // reason entirely.
    mockPrisma.leaseRentInvoice.findUnique.mockResolvedValue(
      voidedRow({ amountDue: new Prisma.Decimal(0) }),
    );
    await expect(service.recordPayment('inv-1', { amountPaid: 100 })).rejects.toThrow(
      /voided when the tenancy ended/,
    );
  ...
  it('still lets a WAIVED month be cleared — that decision stays reversible', async () => {
    mockPrisma.leaseRentInvoice.findUnique.mockResolvedValue(
      invoiceRow({ status: 'WAIVED', notes: 'Waived: goodwill' }),
    );
    ...
    expect(cleared.status).toBe('DUE');
    expect(mockPrisma.leaseRentInvoice.update).toHaveBeenCalledTimes(1);
  });

  it('leaves the ordinary DUE -> PAID path untouched', async () => {
    mockPrisma.leaseRentInvoice.findUnique.mockResolvedValue(invoiceRow());
  ...
  it('keeps a voided month out of AR, which is the whole point of refusing', async () => {
    // The end state the guard protects: VOID drops out of `billed`, so an early
    // termination does not leave the rest of the term sitting in outstanding.
    const summary = summariseInvoices(
      [
        { amountDue: 5000, amountPaid: 5000, status: 'PAID', dueDate: d('2026-03-01') },
        { amountDue: 5000, amountPaid: 0, status: 'VOID', dueDate: d('2026-04-01') },
        { amountDue: 5000, amountPaid: 0, status: 'VOID', dueDate: d('2026-05-01') },
      ] as any,
      d('2026-06-01'),
    );
    ...
    // Un-voiding April and May would have put 10,000 back here.
    expect(summary.billed).toBe(5000);
  });
});
...
// ---------------------------------------------------------------------------
// paidAfter / voidAfter (T1.2)
//
// The two money guards on endTenancy. Only ever MOCKED in leases.service.spec, so
// until now nothing tested what they actually query or write — and between them they
// decide whether a departed tenant keeps being invoiced, and whether an invoice with
// real money against it can be wiped.
// ---------------------------------------------------------------------------
describe('LeaseRentInvoiceService.paidAfter — the hard stop', () => {
  let service: LeaseRentInvoiceService;
  ...
  it('looks for money STRICTLY after the move-out month', async () => {
    // gt, not gte: the month the tenant moved out in was legitimately owed and often
    // legitimately paid. Flagging it would refuse every normal end-of-month move-out.
    await service.paidAfter('lease-1', d('2026-06-30'));

    const where = mockPrisma.leaseRentInvoice.findMany.mock.calls[0][0].where;
    expect(where.leaseId).toBe('lease-1');
    expect(where.periodMonth).toEqual({ gt: d('2026-06-30') });
  });

  it('counts a row as paid by EITHER its status or a non-zero amount', async () => {
    // Status and amountPaid can disagree — a partially-cleared row, a hand-edited
    // status. Either signal means real money is attached, and either must block.
    await service.paidAfter('lease-1', d('2026-06-30'));
  ...
  it('returns the offending months oldest first, so the error can name them', async () => {
    expect(mockPrisma.leaseRentInvoice.findMany.mock.calls).toHaveLength(0);
    await service.paidAfter('lease-1', d('2026-06-30'));
    expect(mockPrisma.leaseRentInvoice.findMany.mock.calls[0][0].orderBy)
      .toEqual({ periodMonth: 'asc' });
  });
});

describe('LeaseRentInvoiceService.voidAfter', () => {
  let service: LeaseRentInvoiceService;
  ...
  it('voids rather than deletes, because generation would just recreate them', async () => {
    // Generation is idempotent on (leaseId, periodMonth). A deleted invoice comes
    // straight back on the next run; a voided one is the only thing that sticks.
    const count = await service.voidAfter('lease-1', d('2026-06-30'), 'Tenancy ended');

    expect(mockPrisma.leaseRentInvoice.deleteMany).not.toHaveBeenCalled();
    expect(mockPrisma.leaseRentInvoice.updateMany).toHaveBeenCalled();
    expect(count).toBe(3);
  });

  it('touches only DUE, PARTIAL and FREE months', async () => {
    // PAID is absent by construction: paidAfter has already refused that case, and a
    // void that could reach a paid row would erase a collection.
    await service.voidAfter('lease-1', d('2026-06-30'), 'Tenancy ended');

    const where = mockPrisma.leaseRentInvoice.updateMany.mock.calls[0][0].where;
    expect(where.status).toEqual({ in: ['DUE', 'PARTIAL', 'FREE'] });
  });

  it('refuses to touch any row carrying money, even a DUE one', async () => {
    // Belt to paidAfter's braces. A future caller that skips the guard still cannot
    // void money by accident.
    await service.voidAfter('lease-1', d('2026-06-30'), 'Tenancy ended');
    expect(mockPrisma.leaseRentInvoice.updateMany.mock.calls[0][0].where.amountPaid)
      .toEqual({ lte: 0 });
  });

  it('always stamps voidedAt, which the DB CHECK requires', async () => {
    // invoice_void_requires_voided_at rejects a VOID with no date, so omitting it
    // would surface as an opaque driver error mid-transaction.
    await service.voidAfter('lease-1', d('2026-06-30'), 'Tenancy ended 2026-06-30');

    const data = mockPrisma.leaseRentInvoice.updateMany.mock.calls[0][0].data;
    expect(data.status).toBe('VOID');
    expect(data.voidReason).toBe('Tenancy ended 2026-06-30');
    expect(data.voidedAt).toBeInstanceOf(Date);
  });

  it('falls back to a reason rather than storing an empty one', async () => {
    await service.voidAfter('lease-1', d('2026-06-30'), '   ');
    expect(mockPrisma.leaseRentInvoice.updateMany.mock.calls[0][0].data.voidReason)
      .toBe('Tenancy ended');
  });

  it('runs on the caller transaction client when one is passed', async () => {
    const tx: any = { leaseRentInvoice: { updateMany: jest.fn().mockResolvedValue({ count: 2 }) } };

    const count = await service.voidAfter('lease-1', d('2026-06-30'), 'x', tx);

    expect(tx.leaseRentInvoice.updateMany).toHaveBeenCalled();
    expect(mockPrisma.leaseRentInvoice.updateMany).not.toHaveBeenCalled();
    expect(count).toBe(2);
  });
});

describe('summariseInvoices — VOID must leave the billed total', () => {
  it('excludes VOID alongside WAIVED, or early termination inflates AR', async () => {
    // Without this, ending a 5-year lease in year 1 leaves 4 years of voided rent
    // sitting in outstanding AR — a number someone would chase a departed tenant for.
    const rows = [
      { amountDue: 1000, amountPaid: 1000, status: 'PAID', dueDate: d('2026-01-01') },
      { amountDue: 1000, amountPaid: 0, status: 'VOID', dueDate: d('2026-07-01') },
      { amountDue: 1000, amountPaid: 0, status: 'WAIVED', dueDate: d('2026-08-01') },
    ];

    const summary = summariseInvoices(rows as any, d('2026-12-01'));

    expect(summary.billed).toBe(1000);
    expect(summary.outstanding).toBe(0);
  });

  it('never counts a VOID month as overdue', async () => {
    const rows = [{ amountDue: 1000, amountPaid: 0, status: 'VOID', dueDate: d('2026-01-01') }];
    expect(summariseInvoices(rows as any, d('2026-12-01')).overdueCount).toBe(0);
  });
});

// ---------------------------------------------------------------------------
// Holdover (Phase B) — occupancy past the contracted end
//
// Client decision 2026-08-13: bill the SAME rent, notify leadership. The rate is a
// per-lease percentage so 125–150% is later data entry, not a code change.
//
// The restraint is the important half: the system cannot tell a real holdover from a
// lease nobody closed, and an invoice is PERMANENT once generated.
// ---------------------------------------------------------------------------
describe('holdover billing', () => {
  let service: LeaseRentInvoiceService;

  const TERM_END = d('2026-06-30');
  const HOLDOVER_LEASE = {
    id: 'lease-1',
    leaseStart: d('2026-01-01'),
    leaseEnd: TERM_END,
    monthlyRent: new Prisma.Decimal(5000),
    ...
    terminationDate: null,
    status: 'ACTIVE',
    holdoverRatePct: new Prisma.Decimal(100),
  };
  const PERIOD = [flatPeriod('2026-01-01', '2026-06-30', 5000)];

  beforeEach(() => {
    jest.clearAllMocks();
    service = makeService();
    mockPrisma.lease.findUnique.mockResolvedValue(HOLDOVER_LEASE);
    mockPrisma.leaseRentPeriod.findMany.mockResolvedValue(PERIOD);
    mockPrisma.leaseRentInvoice.createMany.mockResolvedValue({ count: 0 });
    mockPrisma.leaseRentInvoice.findMany.mockResolvedValue([]);
  });

  const monthsBilled = () =>
    (mockPrisma.leaseRentInvoice.createMany.mock.calls[0]?.[0].data ?? []).map((r: any) =>
      day(r.periodMonth),
    );

  it('bills the months after the term ends, at the same rent', async () => {
    await service.generateForLease('lease-1', { through: d('2026-09-15') });

    const rows = mockPrisma.leaseRentInvoice.createMany.mock.calls[0][0].data;
    expect(monthsBilled()).toContain('2026-08-01');
    const august = rows.find((r: any) => day(r.periodMonth) === '2026-08-01');
    expect(money(august.amountDue)).toBe('5000.00');
  });

  it('applies the rate — 150% bills half as much again', async () => {
    mockPrisma.lease.findUnique.mockResolvedValue({
      ...HOLDOVER_LEASE, holdoverRatePct: new Prisma.Decimal(150),
    });
    ...
    const rows = mockPrisma.leaseRentInvoice.createMany.mock.calls[0][0].data;
    const august = rows.find((r: any) => day(r.periodMonth) === '2026-08-01');
    expect(money(august.amountDue)).toBe('7500.00');
  });

  it('bills NOTHING when no rate is set — the default', async () => {
    // The system cannot distinguish a genuine holdover from a lease nobody closed. Both
    // are ACTIVE, past leaseEnd, with no terminationDate. Billing the second would put
    // permanent invoices against a tenant who left months ago.
    mockPrisma.lease.findUnique.mockResolvedValue({ ...HOLDOVER_LEASE, holdoverRatePct: null });

    await service.generateForLease('lease-1', { through: d('2026-09-15') });

    expect(monthsBilled()).not.toContain('2026-08-01');
    expect(monthsBilled()).toContain('2026-06-01'); // contracted months still bill
  });

  it('stops the moment a move-out is recorded', async () => {
    mockPrisma.lease.findUnique.mockResolvedValue({
      ...HOLDOVER_LEASE, terminationDate: d('2026-07-15'),
    });
  ...
  it('does not hold over a lease that was deliberately closed', async () => {
    mockPrisma.lease.findUnique.mockResolvedValue({ ...HOLDOVER_LEASE, status: 'EXPIRED' });
  ...
  it('derives the rate from the last PAYING month, not the last month', async () => {
    // A lease ending in an abatement would otherwise hold the tenant over at £0 —
    // silently free rent forever.
    mockPrisma.leaseRentPeriod.findMany.mockResolvedValue([
      flatPeriod('2026-01-01', '2026-05-31', 5000, 'p1'),
      { ...flatPeriod('2026-06-01', '2026-06-30', 0, 'p2'), isFreeRent: true },
    ]);
  ...
  it('does not double-bill the final contracted month', async () => {
    // The synthetic period starts the day AFTER the term ends, so June is billed once by
    // the real period and not again by the holdover extension.
    await service.generateForLease('lease-1', { through: d('2026-09-15') });

    const june = monthsBilled().filter((m: string) => m === '2026-06-01');
    expect(june).toHaveLength(1);
  });

  it('bills nothing before the term has even ended', async () => {
    await service.generateForLease('lease-1', { through: d('2026-05-15') });
    expect(monthsBilled().every((m: string) => m <= '2026-05-01')).toBe(true);
  });

  it('bills only COMPLETED months — never the current one, pro-rata to today', async () => {
    // Caught end-to-end: with `through` = today, August billed 13/31 of the rent on the
    // 13th. Generation is idempotent on (leaseId, periodMonth), so that partial figure
    // would be PERMANENT — and wrong the moment the month finished. One month later is
    // always right; a locked-in fraction never is.
    await service.generateForLease('lease-1', { through: d('2026-09-15') });

    expect(monthsBilled()).toContain('2026-08-01');   // completed
    expect(monthsBilled()).not.toContain('2026-09-01'); // still running
  });

  it('bills the completed holdover month in FULL, not pro-rata', async () => {
    await service.generateForLease('lease-1', { through: d('2026-09-15') });

    const rows = mockPrisma.leaseRentInvoice.createMany.mock.calls[0][0].data;
    const august = rows.find((r: any) => day(r.periodMonth) === '2026-08-01');
    expect(august.isProrated).toBe(false);
    expect(money(august.amountDue)).toBe('5000.00');
  });

  it('bills nothing when the term ended earlier this month', async () => {
    // Nothing has completed yet, so there is nothing correct to bill.
    mockPrisma.lease.findUnique.mockResolvedValue({
      ...HOLDOVER_LEASE, leaseEnd: d('2026-09-05'),
    });
    mockPrisma.leaseRentPeriod.findMany.mockResolvedValue([
      flatPeriod('2026-01-01', '2026-09-05', 5000),
    ]);

    await service.generateForLease('lease-1', { through: d('2026-09-15') });

    expect(monthsBilled()).not.toContain('2026-10-01');
    expect(monthsBilled().filter((m: string) => m > '2026-09-01')).toHaveLength(0);
  });
});

describe('summariseInvoices — flagged months (R22)', () => {
  const row = (over: Record<string, unknown> = {}) => ({
    amountDue: 1000, amountPaid: 0, status: 'DUE', dueDate: d('2026-01-05'), ...over,
  });

  it('counts months billed from a period that was corrected afterwards', () => {
    const s = summariseInvoices(
      [row({ needsReview: true }), row(), row({ needsReview: true })],
      d('2026-05-15'),
    );
    expect(s.flaggedCount).toBe(2);
  });

  it('counts a flagged month even when it is fully paid — the question is what was billed, not whether the money arrived', () => {
    const s = summariseInvoices(
      [row({ status: 'PAID', amountPaid: 1000, needsReview: true })],
      d('2026-05-15'),
    );
  ...
  it('treats an absent needsReview as not flagged, so callers selecting fewer columns still work', () => {
    expect(summariseInvoices([row()], d('2026-05-15')).flaggedCount).toBe(0);
  });
});
