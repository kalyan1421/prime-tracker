...
async function main() {
  ...
  // ---- Demo Users ----
  // Demo/dev login users (hardcoded IDs matching jwt-auth.guard demo bypass)
  // are owned exclusively by `seed-demo-users.ts`, which sets the @prime.dev
  // emails used by the Dev Quick Login buttons AND their password hashes.
  // Do NOT recreate them here — duplicating the IDs caused upsert collisions.
